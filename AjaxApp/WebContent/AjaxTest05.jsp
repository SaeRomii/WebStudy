...
<script type="text/javascript">
	...
	// 키보드를 누릴 때 마다 그 때 까지의 입력된 단어를 서버로 전송하고,
	...
	function search()
	{
		...
		// 테스트
		//alert(search);  → 검색창에 문자 입력하면 팝업 창에 입력한 문자 나옴
		...
		// 검색 키워드를 전송할 수 있도록 구성 → 정규표현식 사용
		var regEx = /^[가-힣]{1,}$/;	// //<- 안에 들어있는건 문자 아님 정규표현식임
		
		if(!regEx.test(search))		//수식이 맞다면 유효하지 않는다면 종료시켜줘
			return;
	...
	function callBack()
	{
		...
		// items 박스 초기화 (검색어 창에 썼다가 지우면 추천 검색어도 없어지게)
