...
<script type="text/javascript">
	...
	//   → span 만들어놨음 → id:result(id가 result인것)
	function callBack()
	{
		var data = ajax.responseText;	//Test03OK의 result가 이쪽으로 수신됨
	...
	function callBack2()
	{
		...
		// 주소 목록 쫙 있는 곳에서 item 얻어옴
		// 루트 엘리먼트를 활용하여 모든 『<item>』엘리먼트들 반환받기 → select 박스에 넣어주려고
		...
		// 검색 결과 확인
		if(itemList.length==0)  //itemList길이가 0이면 수신된 주소 없는거
		{
			document.getElementById("addrResult").innerHTML
		...
		for(var i=0; i<itemList.length; i++)
		{
			...
			// select box 에 아이템 추가
			// <option value='06252/서울특별시...'>[06252] 서울특별시강남구 강남... </option>
			document.getElementById("addrResult").innerHTML
